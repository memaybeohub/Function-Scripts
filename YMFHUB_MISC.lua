    ...
    while task.wait() do
        if Settings["CuttayV4"] then 
            ...
            if LocalPlayerLevelValue < 2450 then 
                ...
                if Sea1 and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("DressrosaQuestProgress", "Dressrosa") == 0 then 
                ...
                if not Settings["NeedBone"] then MethodFarm_Dropdown:Set("Level") end
                ...
                if LocalPlayerLevelValue >= 1500 and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("TalkTrevor", "1") ~= 0 then 
                    Settings["HopFruit"] = true 
                else 
                    Settings["HopFruit"] = false
                end
                Sharkmanquest = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySharkmanKarate", true)
                DeathStepQuest = game.ReplicatedStorage.Remotes.CommF_:InvokeServer("BuyDeathStep", true)
                if (typeof(Sharkmanquest) ~= "string" and Settings["DeathStepDoor"]) and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ZQuestProgress", "Zou") == 0 then 
                    game.ReplicatedStorage.Remotes.CommF_:InvokeServer("TravelZou") 
                end
                if CheckItem("Yama") and CheckItem("Tushita") and (CheckItem("Yama")["Mastery"] < 350 or CheckItem("Tushita")["Mastery"] < 350) then
                    if CheckItem("Yama")["Mastery"] < 350 and not CheckTool("Yama") then 
                        LoadItemFromInventory("Yama")
                    elseif (not CheckItem("Yama") or CheckItem("Yama")["Mastery"] >= 350) and CheckItem("Tushita")["Mastery"] < 350 and not CheckTool("Tushita") then 
                        LoadItemFromInventory("Tushita")
                    end
                    if BringMobChoosen and string.find(BringMobChoosen.Name,"boss") then
                ...
                if LocalPlayerLevelValue >= 850 and AutoBartiloQuest_Toggle and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") ~= 3 then AutoBartiloQuest_Toggle:Set(true)  elseif AutoBartiloQuest_Toggle then AutoBartiloQuest_Toggle:Set(false) end
                if KillPlayerQuest_Toggle and LocalPlayerLevelValue < 500 then KillPlayerQuest_Toggle:Set(true) elseif KillPlayerQuest_Toggle then KillPlayerQuest_Toggle:Set(false) end
                AutoActiveRace_Toggle:Set(true)
                if AutoSea3_Toggle and game.ReplicatedStorage.Remotes.CommF_:InvokeServer("ZQuestProgress", "Zou") ~= 0 then AutoSea3_Toggle:Set(true) end
                if AutoRaid_Toggle and LocalPlayerFragmentValue < 7500 and not CheckX2Exp() then AutoRaid_Toggle:Set(true) elseif AutoRaid_Toggle and AutoItemStatus ~= "Not Enough Beli/Fragment to buy melee" then  AutoRaid_Toggle:Set(false) end
                if AutoBuyHakiColor_Toggle then AutoBuyHakiColor_Toggle:Set(true) end 
                if AutoBuyHakiColorLegend_Toggle then AutoBuyHakiColorLegend_Toggle:Set(true) end 
...
local WelcomeMessage3 = "<Color=Cyan>Have A Good Day!<Color=/>"
local tickpirateraid = tick()-30
...
function CheckPirateRaidConnect()
    for i,v in pairs(game.Players.LocalPlayer.PlayerGui.Notifications:GetChildren()) do 
        if string.find(v.Text,"approaching") then
            CreateGameNotify("<Color=Red>Founded Pirates Raid...<Color=/>",500)
...
spawn(function()
    while task.wait() do 
        if Settings["Auto Melee"] then 
            if willspark then 
                if AutoItemStatus == "Not Enough Beli/Fragment to buy melee" then
        ...
        if Settings["Auto Melee"] then 
            pcall(function()
                ...
                elseif Settings["Auto Melee"] then 
                    ...
                    elseif Settings["Melee Choosen"]["Sharkman Karate"] and (typeof(game.ReplicatedStorage.Remotes.CommF_:InvokeServer("BuySharkmanKarate", true)) == "string" or Sea2) and willspark["Sharkman Karate"] == 0 then
                    ...
                    elseif Settings["Melee Choosen"]["Godhuman"] and willspark["Godhuman"] == 0 then 
                        ...
                        if #zzzzzzz >=5 then 
                            ...
                            if #materialmfa > 0 then
                                for v,i in pairs(materialmfa) do 
    ...
    function()
        ...
        while wait() do
            ...
            if Settings["Auto Farm Mob Select"] and Settings["Selected Mobs"] and not CheckIsRaiding() then 
                ...
                for i,v in pairs(Settings["Selected Mobs"]) do print(i,v) end
            ...
            elseif Sea3 and Settings["Auto Kill Player Trial"] and GetPlayerKillTrial() then 
                ...
                pcall(function()
                    ...
                    repeat 
                        if FastAttackConnector:GetMethod() == "Slow" or FastAttackConnector:GetCount() >= Settings["CDAAT"] then 
                            UseFastAttack = false
            ...
            elseif Sea3 and Settings["Auto Yama"] and CheckCandoYama() then 
                AutoYama()
            elseif Sea2 and Settings["Auto Rengoku"] and CheckTool("Hidden Key") and not rengokued and not CheckIsRaiding() then 
            ...
            elseif Sea2 and Settings["Auto Third World"] and CheckQuestZQuest() then 
                AutoSea3()
            elseif not Sea1 and Settings["CuttayV4"] and CheckBossFarmEx() then 
                oas = CheckBossFarmEx()
